...
#           sub-method
# =============================================================================
    def write_inp_file_from_template(self, case_directory, sample_directory, inp_template, sammy_inp_filename, solve_bayes):
...
#             sub-method
# =============================================================================
    def write_par_file_from_template(self, isample, case_directory, case_basename, sample_directory, number_of_levels, par_template, sammy_par_filename, csv):
        with open(os.path.join(case_directory, par_template),'r') as f:
            par_template_lines = f.readlines()
            f.close()
            
        sub_method_warning = []
        parameters = pd.read_csv(os.path.join(case_directory,case_basename,csv), index_col=0)
        ...
        for ilevel in range(1,number_of_levels+1):
            energy = str(sample_parms[f'E{ilevel}']); energy = self.pad_res_parms(energy,11)
            Gg = str(sample_parms[f'Gg{ilevel}']); Gg = self.pad_res_parms(Gg,10)
            Gn = str(sample_parms[f'Gn{ilevel}']); Gn = self.pad_res_parms(Gn,10)
...
awk '{print "     " $1 "     " $4 "      " $4}' SAMMY.LST > syndat_"""+str(isample))
...
# =============================================================================
    def run_awk(self,case_directory,case_basename,first_case,last_case):
        ...
        for isample in range(first_case,last_case+1):
            wildcard_path = os.path.join(case_directory, case_basename, f'{case_basename}_smpl_{isample}/awk.sh.*')
        ...
        for i in run_cases:
            directory = os.path.join(case_directory, case_basename,f'{case_basename}_smpl_{i}')
...
# !!! need to update this to take multiple levels
    def pull_parameters_from_bayes_updated_par(self, case_directory, case_basename, number_of_samples):
        ...
        for isample in range(1,number_of_samples+1):
            ...
            if os.path.isfile(bayes_updated_par_file):
                with open(bayes_updated_par_file,'r') as f:
                    ...
                    case_df = pd.DataFrame({'E1':splitline[0], 'Gg1':splitline[1], 'Gn1':splitline[2]}, index=[isample])
            else:
        ...
        sorted_df.to_csv(os.path.join(case_directory,case_basename,'bayes_parameters.csv'))
...
#         PRIMARY METHOD
# =============================================================================
    def create_synthetic_data(self, case_directory, case_basename, number_of_samples, number_of_levels, run_sammy, Truecsv, par_template, inp_template):
        ...
        for isample in range(1,number_of_samples+1):
            ...
            sample_directory = os.path.join(case_directory, f'{case_basename}/{sample_name}')
            ...
            self.write_inp_file_from_template(case_directory, sample_directory, inp_template, sammy_inp_filename, False); inputs_created += 1
            smw2 = self.write_par_file_from_template(isample, case_directory, case_basename, sample_directory, number_of_levels, par_template, sammy_par_filename, Truecsv); par_created += 1
            ...
            shutil.copy(os.path.join(case_directory,E_structure_filename), sample_directory); copied_E_struct += 1
        ...
        if run_sammy:
            ...
            irunsammy = self.run_sammy_and_wait(case_directory, case_basename, number_of_samples)
...
#       PRIMARY METHOD
# =============================================================================
    def run_bayes(self, case_directory, case_basename, number_of_samples, number_of_levels, Baroncsv, par_template, inp_template):
        ...
        for isample in range(1,number_of_samples+1):
            ...
            self.write_inp_file_from_template(case_directory, sample_directory, inp_template, sammy_inp_filename, True); inputs_created += 1
            # write par file in case directory from template
            smw2 = self.write_par_file_from_template(isample, case_directory, case_basename, sample_directory, number_of_levels, par_template, sammy_par_filename, Baroncsv); par_created += 1
        ...
        self.pull_parameters_from_bayes_updated_par(case_directory, case_basename, number_of_samples)
