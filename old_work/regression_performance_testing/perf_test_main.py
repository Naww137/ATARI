    ...
    for peak in loop_peaks:
        for epts in loop_energies:
            ...
            for isample in range(1, number_of_samples + 1):
                ...
                new_matlab_string = new_matlab_string.replace(
                    "%%%TrueNumPeaks%%%", str(TrueNumPeaks)
                )
                new_matlab_string = new_matlab_string.replace(
                    "%%%NumPeaks%%%", str(peak)
                )
                new_matlab_string = new_matlab_string.replace(
                    "%%%EnergyPoints%%%", str(epts)
                )
                new_matlab_string = new_matlab_string.replace(
                ...
                with open(
                    os.path.join(parm_directory, f"{parm_name}_smpl_{isample}.m"), "w"
                ) as matscript:
            ...
            new_sub_jobarray_template = new_sub_jobarray_template.replace(
                "%%%parm_name%%%", parm_name
            )
            new_sub_jobarray_template = new_sub_jobarray_template.replace(
                "%%%array_specs%%%", job_array_specifications
            )
            with open(
                os.path.join(parm_directory, f"{parm_name}_jobarray.sh"), "w"
            ) as subscript:
...
if submit_jobarrays:
    for peak in loop_peaks:
        for epts in loop_energies:
            ...
            parm_directory = os.path.join(case_directory, f"{parm_name}")
            os.system(
                f"ssh -t necluster.ne.utk.edu 'cd {parm_directory} ; qsub {parm_name}_jobarray.sh'"
            )
...
if compile_output_stats:
    ...
    for peak in loop_peaks:
        ...
        for epts in loop_energies:
            ...
            for file in os.listdir(parm_directory):
                if file.endswith(".out"):
                    ...
                    with open(os.path.join(parm_directory, file), "r") as f:
                        ...
                        parm_sample_data["sample"].append(
                            float(re.split("_|\.", f.name)[-2])
                        )
            ...
            parm_case_data.to_csv(
                os.path.join(parm_directory, "parm_case_data.csv"),
                index=True,
                header=True,
            )
            ...
            conv_rate_epts[f"{epts}"] = sum(parm_sample_data["convergence"]) / len(
                parm_sample_data["convergence"]
            )
    ...
    avg_time_data = pd.DataFrame(avg_time)
    avg_time_data.to_csv("avg_time_data.csv", index=True, header=True)
    avg_SE_data = pd.DataFrame(avg_SE)
    avg_SE_data.to_csv("avg_SE_data.csv", index=True, header=True)
    avg_iter_data = pd.DataFrame(avg_iter)
    avg_iter_data.to_csv("avg_iter_data.csv", index=True, header=True)
    avg_conv_data = pd.DataFrame(avg_conv)
    avg_conv_data.to_csv("avg_conv_data.csv", index=True, header=True)
