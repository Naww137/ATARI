...
average_parameters = pd.DataFrame(
    {
        "dE": {"3.0": 8.79, "4.0": 4.99},
        "Gg": {"3.0": 46.4, "4.0": 35.5},
        "gn2": {"3.0": 64.0, "4.0": 64.0},
    }
)

Ta_pair = syndat.particle_pair(
    ac,
    M,
    m,
    I,
...
exp = syndat.experiment(
    E_min_max, input_options=input_options, experiment_parameters=experiment_parameters
)
...
input_options = {"Overwrite Syndats": False, "Overwrite Fits": False, "Use HDF5": False}

perf_test = pf.performance_test(
    number_of_datasets, case_file, input_options=input_options
)

sample_data_df = perf_test.generate_syndats(Ta_pair, exp, solver="syndat_SLBW")
...
print(
    f"Min/Max # Resonances: {np.min(sample_data_df.NumRes)}/{np.max(sample_data_df.NumRes)}"
)
