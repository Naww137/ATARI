...
def get_Vs_at_T(inp, rto, ladder):
    covariance_data_at_theory = get_idc_at_theory(inp, rto, ladder)
    Ds, Vs = get_Ds_Vs(inp.datasets, covariance_data_at_theory, normalization_uncertainty=0.0384200, idc_at_theory=True)
    return Vs
...
def external_derivs(Ds, Vs, Ts, Gs, V_is_inv:bool):
    Npar = Gs[0].shape[1]
    grad = np.zeros((Npar,))
    hess = np.zeros((Npar,Npar))
    chi2s = []
    for D, V, T, G in zip(Ds, Vs, Ts, Gs):
        if V_is_inv:
            Vinv = V
        else:
        ...
        chi2 = DmT.T @ ViDmT
        chi2s.append(chi2)
    return chi2s, grad, hess

def get_derivatives_for_step(rto, Ds, Vs, 
    ...
    if zero_derivs_at_no_vary:
        Gs = [zero_G_at_no_vary(G, res_lad) for G in Gs]

    chi2, dchi2_dpar, hessian_approx = external_derivs(Ds, Vs, Ts, Gs, V_is_inv)
    return chi2, dchi2_dpar, hessian_approx, sammy_pws

def evaluate_chi2_location_and_gradient(rto, Pu, starting_ladder, particle_pair, Ds, Vs, datasets, covariance_data, experiments, inp_for_theory, V_is_inv=False):
    res_lad = get_p_resonance_ladder_from_Pu_vector(Pu, starting_ladder,particle_pair)

    if inp_for_theory is not None:
        Vs = get_Vs_at_T(inp_for_theory, rto, res_lad)
    else: pass # don't need to redefine V

    chi2, dchi2_dpar, hessian_approx, sammy_pws = get_derivatives_for_step(rto, Ds, Vs, datasets, covariance_data, res_lad, particle_pair,experiments, V_is_inv=V_is_inv)
...
def get_chi2(Ds, Vis, pw_lists, experiments):
    Ts = []
    ...
    chi2s = []
    chi2ns = []
    for D, Vi, T in zip(Ds, Vis, Ts):
        print(D.shape, Vi.shape, T.shape)
        chi2 = (D-T).T @ Vi @ (D-T)
        chi2n = chi2 / len(D)
        chi2s.append(chi2)
        chi2ns.append(chi2n)
...
def fit(rto, 
        ...
        external_resonance_indices, 
        particle_pair,
        Ds, Vs, experiments, datasets, covariance_data,
    ...
    for istep in range(steps):
        ...
        chi2s, dchi2_dpar_next, hessian_approx, sammy_pws, res_lad = evaluate_chi2_location_and_gradient(rto, Pu_next, starting_ladder, particle_pair, Ds, Vs, datasets,covariance_data, experiments, inp_for_theory, V_is_inv=V_is_inv)
            ...
            if LevMar:
                ...
                if obj < obj_log[istep-1]:
                    ...
                    while True:  
                        ...
                        chi2s_temp, dchi2_dpar_temp, hessian_approx_temp, sammy_pws_temp, res_lad_temp = evaluate_chi2_location_and_gradient(rto, Pu_temp, starting_ladder, particle_pair, Ds, Vs, datasets,covariance_data,experiments, inp_for_theory, V_is_inv=V_is_inv)
                        total_derivative_evaluations += 1
                        reg_pen_temp, dreg_dpar_temp = get_regularization_location_and_gradient(Pu_temp, ign, iE, iext,
                                                                                                lasso =lasso, lasso_parameters = lasso_parameters,
                                                                                                ridge = ridge, ridge_parameters = ridge_parameters,
                                                                                                elastic_net = elastic_net, elastic_net_parameters = elastic_net_parameters)
                        chi2_temp = np.sum(chi2s_temp)
    ...
    return saved_res_lads, save_Pu, saved_pw_lists, saved_gradients, chi2s_log, obj_log, total_derivative_evaluations
...
def run_sammy_EXT(sammyINP:SammyInputDataEXT, sammyRTO:SammyRunTimeOptions):
    ...
    if sammyINP.V_is_inv and not sammyINP.idc_at_theory:
        ...
        Vs = sammyINP.Vinv
        Ds = sammyINP.D
        chi2, chi2n = get_chi2(Ds, Vs, sammyOUT.pw, sammyINP.experiments_no_pup)
        sammyOUT.chi2 = chi2
        sammyOUT.chi2n = chi2n
    else:
        Ds, Vs = get_Ds_Vs(sammyINP.datasets, 
            sammyINP.experimental_covariance, 
            normalization_uncertainty=sammyINP.cap_norm_unc)
        # D = np.concatenate(Ds)
    
        if sammyINP.remove_V:
            Vs = [np.diag(np.ones(len(D))) for D in Ds]
        # else:
    ...
    if empty:
        ...
        if sammyINP.solution_mode == "SGD":
            saved_res_lads, save_Pu, saved_pw_lists, saved_gradients, chi2_log, obj_log, ibest, total_derivative_evaluations = sgd(sammyRTO, sammyINP.resonance_ladder, sammyINP.external_resonance_indices, sammyINP.particle_pair, Ds, Vs, 
            ...
            saved_res_lads, save_Pu, saved_pw_lists, saved_gradients, chi2_log, obj_log, total_derivative_evaluations = fit(sammyRTO, sammyINP.resonance_ladder, sammyINP.external_resonance_indices, sammyINP.particle_pair, Ds, Vs, 
                                                                                            sammyINP.experiments_no_pup, sammyINP.datasets, sammyINP.experimental_covariance, 
                                                                                            ...
                                                                                            sammyINP.elastic_net, sammyINP.elastic_net_parameters, covs = Vs)
            ...
            if sammyINP.idc_at_theory:
                ...
                Vis = sammyINP.Vinv
                chi2, chi2n = get_chi2(Ds, Vis, sammyOUT_post.pw, sammyINP.experiments_no_pup)
                sammyOUT.chi2_post = chi2
                sammyOUT.chi2n_post = chi2n
            else:
                sammyOUT.chi2_post = sammyOUT_post.chi2
                sammyOUT.chi2n_post = sammyOUT_post.chi2n
    else:
