...
class FitAndEliminate(unittest.TestCase):
    """
    ...
    """
    ...
    def setUpClass(cls):
        ...
        # Syndat models
        syndat_T = Syndat_Model(exp_model_T, Transmission_RPI(), Transmission_RPI(), options = syndatOPT(calculate_covariance=True), title='trans', rng=rng)
        ...
        syndat_Y = Syndat_Model(exp_model_Y, Capture_Yield_RPI(), Capture_Yield_RPI(), options = syndatOPT(calculate_covariance=False), title='cap', rng=rng)
        ...
        ### get data
        cls.eval_data = Evaluation_Data.from_hdf5(experimental_titles=['trans','cap'], experimental_models=[exp_model_T, exp_model_Y], sample_file="./syndat_samples.hdf5", isample=0)
        ...
        cls.initial_feature_bank = initial_feature_bank
...
# print(elimination_history[0]['selected_ladder_chars'].par_post)
        ...
        cls.af = auto_fit.AutoFit(sammy_rto_fit, Ta_pair, solver_options, solver_options, AutoFit_options=autofit_options, fit_and_elim_options=fit_and_elim_options)
        
    def test_fit_and_eliminate(self):
        autofit_out = self.af.fit(self.eval_data, self.initial_feature_bank, fixed_resonance_indices=[])
