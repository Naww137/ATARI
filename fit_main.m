...
% Energies = synth_exp_dat(415:2296,1)' ;
% true_xs = synth_exp_dat(415:2296,2)';
...
if plotting
    figure(icase); clf
    plot(Energies,true_xs, '.', 'DisplayName', 'Synth Exp Data'); hold on
    if add_noise
...
WC = true_xs; 
WE = Energies;
[w, xs_func, SE] = run_baron(NumPeaks, WC, WE, run_baron_bool, initial_vec); 
...
%     disp(SE(solution_w));disp(SE(initial_vec))
    if run_baron_bool
        plot(Energies, xs_func(w), 'DisplayName','Baron Sol');
%         disp(SE(w))
    end
    legend()
end
...
writetable(T,'baron_parameters.csv')
