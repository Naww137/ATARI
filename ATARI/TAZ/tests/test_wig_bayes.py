...
class TestBayesSampler2SG(unittest.TestCase):
    ...
    def test_poisson(self):
        """
        Here, we intend to verify that WigBayes returns the prior when provided Poisson distributions.
        """
...
""")
    ...
    def test_probability_frequency(self):
        """
        Here, we intend to verify that WigBayes returns probabilities that match the fraction of resonances with said probability within statistical error.
        """
        prior, log_likelihood_prior = PTBayes(self.res_ladder, self.reaction)
...
class TestBayesSampler1or2SG(unittest.TestCase):
    ...
    def test_1_2_sg_match(self):
        ...
        runmaster1 = RunMaster(E, EB, distributions1, false_dens, prior1, log_likelihood_prior1, err=self.err)
        posterior1 = runmaster1.WigBayes()
        
        prior2, log_likelihood_prior2 = PTBayes(res_ladder, reaction2)
        distributions2 = reaction2.distributions(dist_type='Wigner')
        runmaster2 = RunMaster(E, EB, distributions2, false_dens, prior2, log_likelihood_prior2, err=self.err)
...
class TestBayesSampler2or3SG(unittest.TestCase):
    ...
    def test_2_3_sg_match(self):
        """
        Here, we intend to verify that the 2-spingroup Encore algorithms converges to the 3-spingroup Encore algorithms when the one spingroup is very infrequent (i.e. low level-density).
        """
...
class TestBayesSamplerNoFalse(unittest.TestCase):
    """
    The purpose of this test is to verify that the WigBayes algorithm is working correctly when no false resonances are present.
    """
    ...
    def test_false(self):
        """
        Here, we intend to verify that WigBayes returns near-zero false probabilities when the false level-density is zero.
        """
        prior, log_likelihood_prior = PTBayes(self.res_ladder, self.reaction)
        distributions = self.reaction.distributions(dist_type='Wigner')
        runmaster = RunMaster(self.E, self.EB, distributions, self.false_dens, prior, log_likelihood_prior, err=self.err)
...
class TestBayesSamplerNoFalseSmall(unittest.TestCase):
    """
    The purpose of this test is to verify that the WigBayes algorithm is working correctly when no false resonances are present.
    """
