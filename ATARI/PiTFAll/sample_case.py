...
def read_fit_datasets(case_file, i):
    ...
    if use_hdf5:
        with h5py.File(case_file, "r") as f:
            fit_pw_df = pd.DataFrame(
                f[f"sample_{i}/fit_pw"][()].T, columns=["E", "est_trans"]
            )
            fit_par_df = pd.DataFrame(
                f[f"sample_{i}/fit_par"][()].T, columns=["E", "Gg", "gnx2"]
            )
            f.close()
    else:
...
def read_syndat_datasets(case_file, i):
    ...
    if use_hdf5:
        ...
        try:
            syndat_par_df = pd.read_csv(
                os.path.join(case_file, f"sample_{i}/syndat_par.csv")
            )
        except:
...
def take_syndat_spingroups(syndat_par_df, fit_par_df):
    if all(item in fit_par_df.columns for item in ["J", "chs", "lwave", "J_ID"]):
        ...
        fit_par_df[["J", "chs", "lwave", "J_ID"]] = syndat_par_df[
            ["J", "chs", "lwave", "J_ID"]
        ]
    return
...
def calculate_xs(energy, Ta_pair, syndat_par_df, fit_par_df, finegrid_bool):
    ...
    xs_tot_syndat, xs_scat_syndat, xs_cap_syndat = scattering_theory.SLBW(
        egrid, Ta_pair, syndat_par_df
    )
    take_syndat_spingroups(syndat_par_df, fit_par_df)
    xs_tot_fit, xs_scat_fit, xs_cap_fit = scattering_theory.SLBW(
        egrid, Ta_pair, fit_par_df
    )
...
def analyze_fit(case_file, isample, particle_pair):
    ...
    xs_tot_syndat, xs_tot_fit, new_egrid = calculate_xs(
        energy, particle_pair, syndat_par_df, fit_par_df, True
    )
    est_sol_SE = integrate.trapezoid((xs_tot_syndat - xs_tot_fit) ** 2, new_egrid)

    FoMs = pd.DataFrame(
        {
            "fit_exp": [est_SE, est_chi_square, est_chi_square / dof],
...
def plot(case_file, isample, particle_pair, fit_bool, finegrid_bool):
    if fit_bool:
        ...
        # convert to xs
        xs_tot_syndat, xs_tot_fit, new_egrid = calculate_xs(
            pw_data.E, particle_pair, syndat_par_df, fit_par_df, finegrid_bool
        )
        # create plot
        ...
        ax[0].plot(
            pw_data.E, pw_data.theo_trans, lw=2, color="g", label="sol", zorder=1
        )
        ...
        ax[1].set_ylabel("Total Cross Section")
        ax[1].legend()
        fig.tight_layout()
