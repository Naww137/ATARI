...
def compute_theoretical(solver, experiment, particle_pair, resonance_ladder, theo_pw, sammy_RTO, sammy_INP):
    ...
    if solver == 'syndat_SLBW':
        xs_tot, xs_scat, xs_cap = SLBW(energy_grid, particle_pair, resonance_ladder)
...
#         sammy_interface.create_sammyinp(os.path.join(realization_dir,'sammy.inp')); inputs_created+=1
#         sammy_interface.create_sammypar(Jn_ladders, Jp_ladders,os.path.join(realization_dir,'sammy.par')); par_created+=1
#     #   could maybe sample a paremter for energy structure, i.e. detector deadtime
#         sammy_interface.write_estruct_file(Estruct, os.path.join(realization_dir,'estruct')); estruct_created+=1
...
#         sammy_interface.write_estruct_file(energy, os.path.join(sample_directory,estruct_filename))
#         sammy_interface.create_sammyinp(filename=os.path.join(sample_directory,sammy_inp_filename), template=inp_template_file)
#         sammy_interface.samtools_fmtpar(sample_array, os.path.join(sample_directory,sammy_par_filename))
