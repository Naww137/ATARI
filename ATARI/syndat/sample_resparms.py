...
def sample_resonance_ladder(Erange, spin_groups, average_parameters, 
        ...
        resonance_ladder = pd.DataFrame({'E':[], 'Gg':[], 'Gn1':[], 'J':[], 'chs':[], 'lwave':[], 'J_ID':[]})
        J_ID = 0
        for ij, j in enumerate(spin_groups):
            ...
            [levels, level_spacing] = sample_RRR_levels(Erange, average_parameters[str(j[0])]["<D>"], ensemble=ensemble, rng=rng)
            ...
            red_gwidth = sample_RRR_widths(levels, average_parameters[str(j[0])]["<Gg>"]/2, average_parameters[str(j[0])]["g_dof"], rng=rng)
            ...
            nwidth = sample_RRR_widths(levels, average_parameters[str(j[0])]["<Gn>"], average_parameters[str(j[0])]["n_dof"], rng=rng)
            E_Gn_gnx2 = pd.DataFrame([levels, Gwidth, nwidth, [j[0]]*len(levels), [j[1]]*len(levels), [j[2]]*len(levels), [J_ID]*len(levels)], index=['E','Gg', 'Gn1', 'J', 'chs', 'lwave', 'J_ID'])  
