...
class performance_test:
    def __init__(self, number_of_datasets, case_file, input_options={}):
        ### Gather inputs
    ...
    ###
    def generate_syndats(self, particle_pair, experiment, solver="syndat_SLBW"):
        ### generate syndats
        syndat.MMDA.generate(
            particle_pair,
            experiment,
            solver,
        ...
        sample_data_df = pd.DataFrame(
            {"NumRes": NumRes, "NumEpts": NumEpts, "theo_exp_SE": theo_exp_SE}
        )
        ...
        if self.use_hdf5:
            ...
            sample_data_df.to_csv(
                os.path.join(self.case_file, "test_stats/sample_data.csv")
            )
    ...
    ###
    def generate_fits(
        self, run_local, path_to_application_exe=None, path_to_fitting_script=None
    ):
        ### Check input
        if run_local:
            if (path_to_application_exe is None) or (path_to_fitting_script is None):
                raise ValueError(
                    "User chose to run fitting algorithm locally but did not supply the proper paths."
                )
            else:
                pass
        ...
        if self.use_hdf5:
            samples_not_being_run = self.prepare_and_fit_hdf5(
                run_local, path_to_application_exe, path_to_fitting_script
            )
        else:
            samples_not_being_run = self.prepare_and_fit_csv(
                run_local, path_to_application_exe, path_to_fitting_script
            )

        # TODO: if not running locally, generate a jobarray.sh with appropriate isamples and print breif instructions
        if not run_local:
            samples_to_be_run = np.setdiff1d(
                np.arange(0, self.number_of_datasets), np.array(samples_not_being_run)
            )
...
Please run samples {min_value}-{self.number_of_datasets}"
    ...
    ###
    def prepare_and_fit_csv(
        self, run_local, path_to_application_exe, path_to_fitting_script
    ):
        ...
        for i in range(self.number_of_datasets):
            ...
            if os.path.isdir(sample_directory):
                ...
                if os.path.isfile(syndat_pw) and os.path.isfile(syndat_par):
                    ...
                    raise ValueError(
                        f"Syndat in sample directory {os.path.abspath(sample_directory)} does not exist."
                    )
                # check for fits
                ...
                if os.path.isfile(fit_pw) and os.path.isfile(fit_par):
                    if self.overwrite_fits:
                        out = self.run_fitting_algorithm(
                            self.case_file,
                            i,
                            run_local,
                            path_to_fitting_script,
                            path_to_application_exe,
                            self.use_hdf5,
                        )
                    else:
                ...
                raise ValueError(
                    f"Sample directory {os.path.abspath(sample_directory)} does not exist."
                )

        return samples_not_being_run

    ###
    def prepare_and_fit_hdf5(
        self, run_local, path_to_application_exe, path_to_fitting_script
    ):
        samples_not_being_run = []
        for i in range(self.number_of_datasets):
            ...
            if sample_group in f:
                # check for syndat
                if ("syndat_pw" in f[sample_group]) and (
                    "syndat_par" in f[sample_group]
                ):
                    pass
                else:
                    raise ValueError(
                        f"Syndat in sample group {sample_group} does not exist in {self.case_file}."
                    )
            ...
            # raise error if sample group does not exist
                ...
                raise ValueError(
                    f"Sample group {sample_group} does not exist in {self.case_file}."
                )
        # close hdf5 file
        f.close()
    ...
    def run_fitting_algorithm(
        ...
        if use_hdf5:
            if run_local:
                fitting_script = os.path.splitext(
                    os.path.basename(path_to_fitting_script)
                )[0]
                fitting_script_directory = os.path.dirname(path_to_fitting_script)

                out = subprocess.run(
                    [
                        f"{path_to_application_exe}",
                        "-nodisplay",
                        "-batch",
                        f'{fitting_script}("{case_file}",{isample})',
                    ],
                    cwd=f"{fitting_script_directory}",
                    check=False,
                    encoding="utf-8",
                    capture_output=True,
                )
            ...
            raise NotImplementedError(
                "Need to update run_local capabilities for use_hdf5=False"
            )
            # if run_local:
    ...
    def analyze(self, particle_pair):
        ...
        for i in range(self.number_of_datasets):
            ...
            fit_exp_SE.append(FoM.fit_exp.SE)
            fit_exp_chi2.append(FoM.fit_exp.Chi2)
            fit_exp_chi2dof.append(FoM.fit_exp["Chi2/dof"])
            theo_exp_SE.append(FoM.theo_exp.SE)
            theo_exp_chi2.append(FoM.theo_exp.Chi2)
            theo_exp_chi2dof.append(FoM.theo_exp["Chi2/dof"])

        integral_FoMs_df = pd.DataFrame(
            {
                "fit_theo_MSE": fit_theo_MSE,
        ...
        if self.use_hdf5:
            ...
            sample_data_df = pd.read_hdf(
                self.case_file, "test_stats/sample_data"
            )  # read out sample data so it can be returned with self.analyze
        else:
            integral_FoMs_df.to_csv(
                os.path.join(self.case_file, "test_stats/integral_FoMs.csv")
            )
