...
true_parameters = pd.read_csv(
    case_directory + "/true_parameters.csv"
)  # pd.read_csv(os.path.realpath("True_Parameters_(E,Gg,Gn).csv"))
baron_parameters = pd.read_csv(
    case_directory + "/baron_parameters.csv"
)  # pd.read_csv(os.path.realpath("Baron_parameters.csv"))
...
plt.xlabel("True Gn")
plt.ylabel("Baron Pred. Gn")
plt.title("With ss w/o edge effects experimental corrections")
...
if has_bayes_solutions:
    summary_statistics = pd.DataFrame(
        {
            "Min Baron Error": min_vec_baron[1:],
            "Max Baron Error": max_vec_baron[1:],
            "Min Bayes Error": min_vec_bayes[1:],
            "Max Bayes Error": max_vec_bayes[1:],
        },
        index=["E", "Gg", "Gn"],
    )
